  ...
  return (
    <>
      ...
      >
        ...
        >
          <EllipsisVerticalIcon className="h-6 group-hover:w-6 group-hover:opacity-100 w-0 opacity-0 transition-all ease-in-out duration-100 text-[#212A3E]" />
          <div
            className={classNames(
              "z-20 text-left absolute bottom-0 translate-y-full text-dark w-52 bg-gradient-to-tr from-indigo-200 to-indigo-100 shadow-lg  rounded-2xl p-1 shadow-md border-[1px] border-zinc-300",
          ...
          >
            {chat.isGroupChat ? (
              ...
              >
                <InformationCircleIcon className="h-6 w-4  " /> About group
              </p>
            ) : (
              <p
                onClick={(e) => {
                ...
                role="button"
                className="p-4 text-danger rounded-lg w-full inline-flex items-center bg-danger text-white"
              >
              ...
              {chat.participants.slice(0, 3).map((participant, i) => {
                return (
                  <img
                    ...
                    className={classNames(
                      "w-7 h-7  rounded-full absolute outline outline-1 outline-dark group-hover:outline-secondary",
                      i === 0
                        ? "left-0 z-[3]"
                        : i === 1
                        ? "left-2.5 z-[2]"
        ...
        <div className="w-full">
          ...
          <div className="w-full inline-flex items-center text-left">
            ...
            <small className="text-[#212A3E] truncate-1 text-sm text-ellipsis inline-flex items-center">
              {getChatObjectMetadata(chat, user!).lastMessage}
            </small>
          </div>
        </div>
        <div className="flex text-dark h-full text-sm flex-col justify-between items-end">
