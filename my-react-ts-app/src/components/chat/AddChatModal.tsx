...
const AddChatModal: React.FC<{
  ...
  const getUsers = async (role:string) => {
    ...
    requestHandler(
      ...
      async () => await getAvailableUsers(role),
  ...
  return (
    <Transition.Root show={open} as={Fragment}>
      <Dialog
        as="div"
        className="relative z-10 bg-[#778899]"
        onClose={handleClose}
      >
        <Transition.Child
          as={Fragment}
          ...
          leaveTo="opacity-0"
        >
          <div className="fixed inset-0 bg-[#FFFAFA] bg-opacity-75 transition-opacity" />
        </Transition.Child>
        ...
        <div className="fixed inset-0 z-10 overflow-y-visible">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <Dialog.Panel
                className="relative transform overflow-x-hidden rounded-lg bg-[#FFFAFA] px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-3xl sm:p-6"
                style={{
                  overflow: "inherit",
                }}
              >
                <div>
                  <div className="flex justify-between items-center">
                    ...
                    <button
                      type="button"
                      className="rounded-md bg-transparent  text-dark-400 hover:text-dark-600 focus:outline-none focus:ring-1 focus:ring-white focus:ring-offset-2"
                      onClick={() => handleClose()}
                    >
                ...
                <div>
                  <Switch.Group as="div" className="flex items-center my-5">
                    <Switch
                      ...
                      className={classNames(
                        isGroupChat ? "bg-zinc-400" : "bg-zinc-200",
                  ...
                  {isGroupChat ? (
                    <div className="my-5">
                      <span
                        className={classNames(
                          "font-medium text-dark inline-flex items-center text-dark"
                        )}
                      >
                        <UserGroupIcon className="h-5 w-5 mr-2 text-dark " />{" "}
                      ...
                      <div className="flex justify-start items-center flex-wrap gap-2 mt-3">
                        {users
                          ...
                          ?.map((participant) => {
                            return (
                              <div
                                className="inline-flex bg-[#FFFAFA] rounded-full p-2 border-[1px] border-zinc-400 items-center gap-2"
                                key={participant._id}
                              >
                                <img
                                ...
                                <XCircleIcon
                                  role="button"
                                  className="w-6 h-6 hover:text-danger cursor-pointer"
                                  onClick={() => {
                                    setGroupParticipants(
                ...
                <div className="mt-5 flex justify-between items-center gap-4  ">
                  ...
                  <Button
                    ...
                    className="w-1/2 bg-gradient-to-tr from-indigo-200 to-indigo-100 text-[#212A3E]"
                  >
                    Create
                  </Button>
                </div>
