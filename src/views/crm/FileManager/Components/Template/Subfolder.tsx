  ...
  useEffect(() => {
    const fetchDataAndLog = async () => {
      try {
        ...
        setTemplateData(templateData.filter((item)=>(item.files[0].folder_name===type && item.files[0].sub_folder_name_first===folderName)).map((item)=>item.files[0]));
      } catch (error) {
      ...
      () => [
          { header: 'Name', accessorKey: 'folder_name', cell: ({row}) =>
             <div className="flex items-center">
              ...
              <Link to={`/app/crm/fileManager/project/templates/residential/subfolder/files?type=${type}&folder=${folderName}&subfolder=${row.original.sub_folder_name_second}&folder_id=${row.original.folder_id}`}>
              {row.original.sub_folder_name_second}</Link></div>} ,
          ...
          { header: 'modified', accessorKey: 'updated_date',
            cell:({row})=>{
           ...
           },
           {
            ...
            accessorKey:'action',
            cell:({row})=>{
                return (<div className=' flex justify-center' onClick={()=>openDialog2(row.original.sub_folder_name_second)}>
